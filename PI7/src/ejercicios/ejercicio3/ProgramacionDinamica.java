...
public class ProgramacionDinamica {
	...
	public static Solucion pd(Integer tProdInicial, Integer tElabInicial) {
		ProgramacionDinamica.maxValue = Integer.MIN_VALUE;
		ProgramacionDinamica.start = new Vertice(0, tProdInicial, tElabInicial);
	...
	public static Sp pd(Vertice vertex, Integer accumulateValue, Map<Vertice, Sp> memory) {
		...
		if (memory.containsKey(vertex)) {
			...
			for (Integer a : vertex.actions()) {
				Integer precio = Modelo.getPrecio(vertex.index());
				Double cota = accumulateValue + Heuristica.dimension(vertex, a);

				if (cota < ProgramacionDinamica.maxValue) {
					continue;
				}

				Sp s = pd(vertex.neighbor(a), accumulateValue + a * precio, memory);

				if (s != null) {
					Sp sp = new Sp(a, s.weight() + a * precio);
					soluciones.add(sp);
				}
