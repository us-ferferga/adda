...
public record Vertice(Integer index, List<Integer> as, Integer presupRestante)
	...
	public static void from(List<String> ls) {
		Vertice.cualidadesACubrir = new Modelo(ls).cualidadesDeseadas.stream().collect(Collectors.toSet());
	}
	...
	public static Vertice of(Integer index, List<Integer> as, Integer presupRestante) {
		return new Vertice(index, as, presupRestante);
	}
	...
	public List<Integer> actions() {
		...
		if (Modelo.candidatos.get(index).sueldoMin() <= presupRestante) {
	...
	public Vertice neighbor(Integer a) {
		...
		if (a == 1) {
			as2.add(index);
			cualidades2.removeAll(Modelo.candidatos.get(index).cualidades());
			presup2 = (presup2 - Modelo.candidatos.get(index).sueldoMin().intValue());
		}
